...
def get_all_links():
    ...
    print(soup)
    for i in soup.findAll('h3',{'class':"item_header_title"}):
        # for j in range(len(i.contents)):
        #     links.append([i.contents[j].string,i.contents[j].get("href",'')])
        links.append([i.contents[1].string,i.contents[1].get("href",'')])
    index_filename = 'index.txt'  # index.txt中每行是'网址 对应的文件名'
    index = open(index_filename, 'a')
    for i in links:
        index.write(i[0] + '\t' + i[1] + '\n')
